export default class Scene3 extends Phaser.Scene {
    constructor() {
        ...
        // Define sprite bundles for body parts and other elements
    ...
    createSpriteBundles() {
        return {
            ...
            bodyParts: {
                preloadName: "bodyPartsBundle",
                male: this.createGenderSprites([0, 26, 52], [156, 182, 208], this.limits.bodyHead), // head body
                female: this.createGenderSprites([78, 104, 130], [234, 260, 286], this.limits.bodyHead), // head body
                eyes: this.createExtraParts([11, 37, this.spriteRowsRight.eyes[0]], this.limits.eyes),
                shadow: [244, 270, 296],
                misc: [390, 391, 392, 393, 394, 395, 396],
            },
            smallparts: {
    ...
    createBundles(leftFrame, centerFrame, rightFrame, limit) {
        ...
        for (let i = 0; i < leftFrame.length; i++) {
            ...
            centerFrames = centerFrames.concat(this.createFrameArray(centerFrame[i], limit));
    ...
    createCombineParts(frame1, frame2, limit) {
        return {
            left: [...this.createFrameArray(frame1[0], limit), ...this.createFrameArray(frame2[0], limit)],
            center: [...this.createFrameArray(frame1[1], limit), ...this.createFrameArray(frame2[1], limit)],
            right: [...this.createFrameArray(frame1[2], limit), ...this.createFrameArray(frame2[2], limit)],
        };
    }

    createExtraParts(partFrame, limit) {
        return {
            left: this.createFrameArray(partFrame[0], limit),
            center: this.createFrameArray(partFrame[1], limit),
            right: this.createFrameArray(partFrame[2], limit)
        };
    }

    createGenderSprites(headFrames, bodyFrames, limit) {
        return {
            head: {
                left: this.createFrameArray(headFrames[0], limit),
                center: this.createFrameArray(headFrames[1], limit),
                right: this.createFrameArray(headFrames[2], limit)
            },
            body: {
                left: this.createFrameArray(bodyFrames[0], limit),
                center: this.createFrameArray(bodyFrames[1], limit),
                right: this.createFrameArray(bodyFrames[2], limit)
            }
        };
    }
    ...
    create() {
        ...
        this.input.keyboard.on('keydown', (event) => {
            if (event.key === 'Backspace' && this.currentCharacter.name.length > 0) {
                this.currentCharacter.name = this.currentCharacter.name.substr(0, this.currentCharacter.name.length - 1);
                if (this.isLockedDesign == false || this.currentCharacter.name == '') {
                    this.changeGender();
                }
            } else if (event.key.length === 1 && this.isNameValid(event.key) && this.currentCharacter.name.length < 12) { // Limit name length to 10 characters
                this.currentCharacter.name += event.key;
                if (this.isLockedDesign == false || this.currentCharacter.name == '') {
                    this.changeGender();
                }
            }

            this.nameText.setText(this.currentCharacter.name || 'Please input a name.');
        });
    }
    ...
    renderCreateCharacter() {
        ...
        // Determine Sprite Indices
        const indices = {
            bodyHead: this.currentCharacter.bodyHead ?? this.randomizer(this.spriteBundles.bodyParts[gender].head.right.length - 1),
        ...
        // Sprite Values
        const bodyHead = {
            head: this.spriteBundles.bodyParts[gender].head.right[indices.bodyHead],
            body: this.spriteBundles.bodyParts[gender].body.right[indices.bodyHead],
        ...
        Object.entries(bodyHead).forEach(([name, value]) => {
            ...
            const rows = this.spriteRowsRight[rowName];
        ...
        this.renderSprite(bodyHead.body, "charBody", this.spriteBundles.bodyParts.preloadName);
        this.renderSprite(bodyHead.head, "charHead", this.spriteBundles.bodyParts.preloadName);
        ...
        this.renderSprite(bodyHead.shirts, "charShirts", names.shirts);
    ...
    changeCharacterColor() {
        ...
        this.currentCharacter.bodyHead = this.randomizer(this.spriteBundles.bodyParts[this.currentCharacter.gender].head.right.length - 1);
    ...
    // Helper method to update character part
    updateCharacterPart(spriteArray, partName, characterPart) {
        const newValue = this.randomizerV2(
            spriteArray,
            characterPart.rowValue, // Keep rowValue unchanged
            this.limits[partName]
        );

        const newIndex = spriteArray.indexOf(newValue);
        if (newIndex !== -1) {
            characterPart.value = newIndex;
        } else {
            console.error(`New ${partName} value is not valid.`);
        }
    }
    ...
    randomizerV2(arrayOfData, startValue, length) {
        const startIndex = arrayOfData.indexOf(startValue);
        if (startIndex === -1) {
            return arrayOfData[0]; // or handle it as needed
        }
        const endIndex = Math.min(startIndex + length, arrayOfData.length);
        const result = arrayOfData.slice(startIndex, endIndex);
        // console.log("Sliced Array:", result);
        return Phaser.Math.RND.pick(result);
    }
    ...
    renderSprite(frame, subjectName, preloadName) {
    ...
    renderMiscButtons(height) {
        ...
        if (!this.isLockedDesign || this.currentCharacter.name === '') {
            ...
            // Paint icon
            this.paintSprite = this.add.sprite(60, -height / 2 + padding + 60, this.spriteBundles.bodyParts.preloadName)
                .setFrame(392)
                .setInteractive();
            ...
            // Gender icon
            var frame = this.currentCharacter.gender === "male" ? this.spriteBundles.genderIcon.male : this.spriteBundles.genderIcon.female;
            this.genderSprite = this.add.sprite(-60, -height / 2 + padding + 60, this.spriteBundles.bodyParts.preloadName)
                .setFrame(frame)
                .setInteractive();
    ...
    changeGender() {
        ...
        this.currentCharacter.bodyHead = null;
